# Canonical Corridor Registry

**Version:** Phase II · 2026-05-17  
**Status:** Source of truth for atlas geography  
**Dataset:** `data/studium_atlas_master.csv` (266 nodes)

---

## Purpose

Corridors are **knowledge transmission geographies**, not tourist regions. Each corridor groups place-rooted crafts and land stewardship practices that share ecological, cultural, or historical continuity. The map displays corridors as **living systems**, not destination pins.

---

## Growth sequence (strategic model)

| Phase | Scale | Role |
|-------|-------|------|
| **3** | Founder signal | Proof of transmission; first illuminated nodes |
| **5** | Regional ring | First complete geographic cluster |
| **13** | Planetary constellation | Founding atlas architecture (13 × 5 = 65 nodes) |
| **50** | Operational atlas | Stable global network |
| **200** | Global knowledge network | Civilizational archive scale |

Corridors expand in **depth** (nodes per corridor) before **breadth** (new corridors).

---

## Core corridors (Founding atlas — Tier I)

### 1. Iberian Craft & Land Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Portugal → Spain → Southern France |
| **Philosophy** | Mediterranean land knowledge meeting centuries-old craft lineages; terraced agriculture, cork forests, stone, and viticulture as interlinked systems |
| **Primary knowledge systems** | Vineyard stewardship, dry stone walling, cork harvesting, ceramics, silversmithing |
| **Archetypes** | Land Stewardship, Food & Fermentation, Architecture & Building Craft, Material Craft |
| **Sensitivity** | Medium (founding anchor: Douro; regenerative land partnerships) |
| **Founding nodes** | Douro Valley, Serra da Estrela, Alentejo, Porto, La Bisbal |
| **Expansion potential** | Basque forge traditions; Catalan ceramics; Provençal land stewardship |
| **Atlas nodes (current)** | 10 |

### 2. Italian Master Craft Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Florence → Carrara → Venice/Murano → Cremona → Tuscany |
| **Philosophy** | Guild-descended mastery; material intelligence in stone, glass, textile, and instrument |
| **Primary knowledge systems** | Textile weaving, marble carving, Murano glass, violin making, leather craft, goldsmithing |
| **Archetypes** | Material Craft, Textile & Fiber Craft, Instrument & Music Craft |
| **Sensitivity** | Low–Medium (established artisan economies; steward review for workshop access) |
| **Founding nodes** | Florence, Carrara, Murano, Cremona, Valenza |
| **Expansion potential** | Sicilian ceramics; Umbrian weaving; Alpine woodcraft |
| **Atlas nodes (current)** | 15 |

### 3. Moroccan Architectural Craft Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Fez → Marrakech → Atlas Mountains → Essaouira |
| **Philosophy** | Architecture as sacred geometry; craft embedded in building culture |
| **Primary knowledge systems** | Zellige tile, tadelakt plaster, Berber weaving, pastoral stewardship, wood carving |
| **Archetypes** | Architecture & Building Craft, Textile & Fiber Craft, Land Stewardship, Material Craft |
| **Sensitivity** | Medium–High (religious craft contexts; steward-led access) |
| **Founding nodes** | Fez, Marrakech, Atlas villages, Essaouira |
| **Expansion potential** | Maghreb cross-links; Andalusian craft continuity |
| **Atlas nodes (current)** | 11 |

### 4. Caucasus Fermentation Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Georgia → Armenia → Eastern Türkiye |
| **Philosophy** | Fermentation as civilization technology; qvevri, copper, carpet, and vineyard as one lineage |
| **Primary knowledge systems** | Qvevri winemaking, clay vessel craft, vineyard stewardship, copper smithing, carpet weaving |
| **Archetypes** | Food & Fermentation, Material Craft, Land Stewardship, Textile & Fiber Craft |
| **Sensitivity** | Medium |
| **Founding nodes** | Kakheti, Tbilisi, Armenian highlands, Anatolia |
| **Expansion potential** | Aegean fermentation links; Balkan craft bridges |
| **Atlas nodes (current)** | 11 |

### 5. Japanese Master Craft Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Kyoto → Nara → Kanazawa → Sado Island → Gifu |
| **Philosophy** | Mastery as lifelong devotion (*shokunin*); material discipline and satoyama ecological integration |
| **Primary knowledge systems** | Indigo dyeing, lacquer, gold leaf, temple carpentry, sword forging, satoyama stewardship |
| **Archetypes** | Material Craft, Textile & Fiber Craft, Architecture & Building Craft, Land Stewardship |
| **Sensitivity** | Medium–High (licensed swordsmiths; temple contexts) |
| **Founding nodes** | Kyoto, Kanazawa, Nara, Sado, Gifu |
| **Expansion potential** | Niigata woodcraft; Okinawan textile traditions |
| **Atlas nodes (current)** | 14 |

### 6. South Asian Textile Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Jaipur → Gujarat → Kutch → Kashmir → Rajasthan |
| **Philosophy** | Textile as encoded memory; dye, weave, and miniature painting as interlinked knowledge |
| **Primary knowledge systems** | Block printing, bandhani, embroidery, shawl weaving, miniature painting |
| **Archetypes** | Textile & Fiber Craft, Material Craft |
| **Sensitivity** | Medium |
| **Founding nodes** | Jaipur, Gujarat, Kutch, Srinagar |
| **Expansion potential** | Bangladesh muslin traditions; Himalayan wool systems |
| **Atlas nodes (current)** | 7 |

### 7. Southeast Asian Dye & Wood Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Yogyakarta → Bali → Northern Thailand → Laos → Java |
| **Philosophy** | Sacred material culture; temple carving, batik, and gamelan as place-bound practice |
| **Primary knowledge systems** | Batik dyeing, temple carving, umbrella craft, silk weaving, gamelan instrument making |
| **Archetypes** | Textile & Fiber Craft, Material Craft, Architecture & Building Craft, Instrument & Music Craft |
| **Sensitivity** | Medium–High (temple and ritual contexts) |
| **Founding nodes** | Yogyakarta, Bali, Chiang Mai, Laos, Java |
| **Expansion potential** | Vietnamese lacquer; Philippine weaving |
| **Atlas nodes (current)** | 6 |

### 8. Andean Knowledge Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Peru → Bolivia → Ecuador → Colombia highlands/Amazon edge |
| **Philosophy** | Vertical ecology; terrace agriculture, textile encoding, and indigenous land ethics |
| **Primary knowledge systems** | Alpaca weaving, terraced agriculture, reed boat building, ikat, herbal medicine, agroforestry |
| **Archetypes** | Textile & Fiber Craft, Land Stewardship, Architecture & Building Craft, Material Craft |
| **Sensitivity** | High (indigenous knowledge; community consent required) |
| **Founding nodes** | Cusco, Sacred Valley, Lake Titicaca, Otavalo, Amazon/Andes Colombia nodes |
| **Expansion potential** | Amazon ethnobotany (steward-gated); Chilean highland links |
| **Atlas nodes (current)** | 19 |

### 9. Mesoamerican Craft & Agriculture Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Oaxaca → Mexico City → Guatemala Highlands → Chiapas |
| **Philosophy** | Pre-colonial agricultural intelligence; weaving and chinampa as living heritage |
| **Primary knowledge systems** | Zapotec weaving, alebrije carving, chinampa agriculture, backstrap weaving, coffee agroforestry |
| **Archetypes** | Textile & Fiber Craft, Land Stewardship, Material Craft, Food & Fermentation |
| **Sensitivity** | High (indigenous lineages; ritual craft) |
| **Founding nodes** | Oaxaca, Xochimilco, Guatemala Highlands, Chiapas |
| **Expansion potential** | Michoacán copper; Yucatán henequen |
| **Atlas nodes (current)** | 11 |

### 10. North American Stewardship Corridor

| Field | Value |
|-------|--------|
| **Geographic span** | Southwest USA → Pacific Northwest → Great Plains → Appalachia → Atlantic coast |
| **Philosophy** | Place-based survival knowledge; indigenous stewardship, arid-land hydrology, and intergenerational craft |
| **Primary knowledge systems** | Adobe architecture, desert water harvesting, cultural burning, canoe carving, bison stewardship, quilting |
| **Archetypes** | Land Stewardship, Architecture & Building Craft, Textile & Fiber Craft, Maritime & Watercraft |
| **Sensitivity** | High (indigenous knowledge; federal trust lands; media restrictions) |
| **Founding nodes** | New Mexico, Arizona, California, Washington, Montana, Gee's Bend, and 50-state USA hub nodes |
| **Expansion potential** | Consolidated from USA state-level hubs into unified corridor taxonomy |
| **Atlas nodes (current)** | 92 |

---

## African expansion corridors (Tier II)

| # | Corridor | Span | Nodes | Sensitivity |
|---|----------|------|-------|-------------|
| 11 | West African Textile & Dye | Ghana → Mali → Burkina Faso → Senegal | 7 | High |
| 12 | Sahel Nomadic Craft | Niger → Chad → Mali | 5 | High |
| 13 | Horn of Africa Agriculture | Ethiopia Highlands | 4 | Medium–High |
| 14 | East African Wood & Land | Kenya → Tanzania → Uganda | 6 | High |
| 15 | Central African Ritual Art | Cameroon → DRC → Gabon | 5 | High |
| 16 | Southern African Craft & Ecology | Zimbabwe → Botswana → South Africa → Madagascar | 7 | Medium–High |

---

## Planetary extension corridors (Tier III — constellation phase)

| Corridor | Span | Nodes | Notes |
|----------|------|-------|-------|
| Scandinavian Craft & Land | Nordic → Baltic | 8 | Timber, textile, land ethics |
| Persian Craft & Garden | Iran | 6 | Garden craft, metalwork, carpet |
| Tibetan Plateau Stewardship | Himalaya | 5 | High-altitude land knowledge |
| Pacific Island Navigation | Oceania | 5 | Wayfinding, maritime tradition |
| Arctic & Subarctic Knowledge | Arctic circle | 6 | Cold-climate survival craft |

---

## Corridor naming rules

1. **Format:** `[Region/Culture] + [Knowledge domain] + Corridor`  
2. **Sub-clusters** (e.g. Tuscany, Kansai) roll up to parent corridor in `studium_atlas_master.csv`; sub-cluster preserved in `Notes`.  
3. **No country-only corridors** unless geographically singular (exception: large unified corridors with internal hubs).  
4. **Renames** require registry update before CSV bulk replace.

---

## Registry maintenance

| Action | Owner | Artifact |
|--------|-------|----------|
| Add corridor | Orchestrator + founder | This file + CSV |
| Add node | Data steward | `studium_atlas_master.csv` |
| Merge duplicate | Data steward | CSV dedupe key: `Corridor + Location + Craft` |
| Change sensitivity | Stewardship council | `08-stewardship-framework.md` |

**Dedupe policy:** Remove only true duplicates (identical corridor, location, craft). Preserve unique offerings.
